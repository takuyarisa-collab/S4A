...
let liveSnapPoint: Vec2 | null = null;
...
function frame(now: number) {
  ...
  if (liveSnapPoint && drawing) {
    ctx.fillStyle = 'rgba(255,255,255,0.95)';
    ctx.beginPath();
    ctx.arc(liveSnapPoint.x, liveSnapPoint.y, 4.5, 0, Math.PI * 2);
    ctx.fill();
  }
...
canvas.addEventListener('pointerdown', (event) => {
  ...
  liveSnapPoint = strokeStartAnchor ? { ...strokeStartAnchor.point } : null;
...
canvas.addEventListener('pointermove', (event) => {
  ...
  if (inputPoints.length > 0) {
    const tipAnchor = nearestStructureAnchor(inputPoints[inputPoints.length - 1]);
    liveSnapPoint = tipAnchor ? { ...tipAnchor.point } : null;
    if (tipAnchor) {
      inputPoints[inputPoints.length - 1] = { ...tipAnchor.point };
    }
  }
